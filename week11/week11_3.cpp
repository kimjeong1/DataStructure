...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        cin >> x >> y;
        int cnt1 = 0, cnt2 = 0;
        if (b.existright(b.search(b.root, x)))
            cnt1 = b.pre(b.search(b.root, x)->right);
        if (b.existright(b.search(b.root, y)))
            cnt2 = b.pre(b.search(b.root, y)->right);
